...
from app import Note, ShareLink, app, db
...
def test_time_format():
    with app.app_context():
        ...
        try:
            ...
            # 转换为UTC并移除时区信息
            expire_at_utc_naive = expire_at.astimezone(timezone.utc).replace(
                tzinfo=None
            )
            ...
            print(
                f"时间比较结果: {expire_at_utc_naive} > {now_utc_naive} = {expire_at_utc_naive > now_utc_naive}"
            )

            # 返回给前端时添加时区信息
            response_expire_at = expire_at_utc_naive.replace(
                tzinfo=timezone.utc
            ).isoformat()

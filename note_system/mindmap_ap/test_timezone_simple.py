...
print(f"转换为本地时间后格式化: {db_time_with_tz.astimezone().strftime('%Y-%m-%d %H:%M')}")

# 测试3：模拟不同时区的转换
print("\n=== 测试3：不同时区转换测试 ===")
import pytz
shanghai_tz = pytz.timezone('Asia/Shanghai')
print(f"转换为上海时间: {db_time_with_tz.astimezone(shanghai_tz).strftime('%Y-%m-%d %H:%M')}")
new_york_tz = pytz.timezone('America/New_York')
print(f"转换为纽约时间: {db_time_with_tz.astimezone(new_york_tz).strftime('%Y-%m-%d %H:%M')}")

# 测试4：模拟模板中的astimezone()调用
print("\n=== 测试4：模板中的astimezone()调用模拟 ===")
template_result = db_time_with_tz.astimezone().strftime('%Y-%m-%d %H:%M')
print(f"模板中使用{{% expire_at.astimezone().strftime('%Y-%m-%d %H:%M') %}}的结果: {template_result}")
...
print(f"4. 对于UTC时间{db_time.strftime('%Y-%m-%d %H:%M')}，用户将看到: {template_result}")

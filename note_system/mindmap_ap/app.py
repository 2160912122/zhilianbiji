...
from flask import Flask, render_template, request, jsonify, abort, \
    send_file, url_for, flash, redirect, session
...
# -------------------- 配置 --------------------
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:123456@localhost:3306/notes_db?charset=utf8mb4'
...
def ai_generate_note():
    ...
    return jsonify({
        'content': content,
        'suggested_title': topic if topic else "AI生成的笔记"
    })
...
class Tag(db.Model):
    ...
    notes = db.relationship('Note', secondary='note_tag', back_populates='tags', passive_deletes=True)
...
class Note(db.Model):
    ...
    # 关联标签
    tags = db.relationship('Tag', secondary='note_tag', back_populates='notes', lazy='dynamic')
...
def get_share_by_token(token):
    ...
    # 检查是否过期 - 使用UTC时间进行比较
    is_expired = False
    if link.expire_at and link.expire_at < datetime.now(timezone.utc).replace(tzinfo=None):
        is_expired = True
...
def get_note_versions(note_id):
    ...
    return jsonify([{
        ...
        'updater': {
            'username': version.updater.username
        },
        'updated_at': version.updated_at.isoformat(),
        'content_preview': version.content[:150] + '...' if len(version.content) > 150 else version.content
...
def create_note():
    ...
    if tag_ids:
        ...
        real_tag_ids = [id for id in tag_ids if not isinstance(id, str) or not id.startswith('temp-')]
        if real_tag_ids:
            tags.extend(Tag.query.filter(Tag.id.in_(real_tag_ids), Tag.user_id == current_user.id).all())
...
def update_note(note_id):
    ...
    title = base_title
    counter = 1
    while Note.query.filter_by(title=title, user_id=current_user.id).first() and title != note.title:
        title = f"{base_title}({counter})"
        counter += 1
...
def create_share():
    ...
    return jsonify({
        'share_url': share_url,
        'permission': permission,
        'expire_at': expire_at.replace(tzinfo=timezone.utc).isoformat() if expire_at else None
    })
...
def get_shares(note_id):
    ...
    for share in shares:
        result.append({
            ...
            'expire_at': share.expire_at.replace(tzinfo=timezone.utc).isoformat() if share.expire_at else None
        })
...
def delete_share(token):
    ...
    note = Note.query.filter_by(id=share.note_id, user_id=current_user.id).first_or_404()
    ...
    if fmt == 'pdf':
        try:
            ...
            if note.type == 'markdown':
                ...
                html_content = (note.content or '').replace('/static/uploads/', f'{base_url}static/uploads/')

...
from app import ShareLink, app, db
...
with app.app_context():
    ...
    link = ShareLink(note_id=14, token=token, permission="edit", expire_at=expire_time)

    db.session.add(link)
    db.session.commit()

    print(
        f"创建分享链接: token={token}, permission={link.permission}, expire_at={link.expire_at}"
    )

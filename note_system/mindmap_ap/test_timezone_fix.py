...
with app.app_context():
    ...
    # 模拟前端时区转换
    local_expire_time = (
        link.expire_at.replace(tzinfo=timezone.utc).astimezone().replace(tzinfo=None)
    )
    ...
    link_1d = ShareLink(
        note_id=note.id, token=token_1d, permission="edit", expire_at=expire_time_1d
    )
    ...
    # 模拟前端时区转换
    local_expire_time_1d = (
        link_1d.expire_at.replace(tzinfo=timezone.utc).astimezone().replace(tzinfo=None)
    )
